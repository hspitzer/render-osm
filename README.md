...
I used Maperitive, a great open steet map (OSM) rendering tool, and hikingmap, a python project that allows the creation of strip maps allong a GPS track.  Additonal tools I used were QMapShack and Inkscape. In the remainder of this post I describe my process for creating the maps to hopefully allow you to create your perfect maps for your next hiking adventure as well.
...
* Maperitive
* hikingmap and hm-render-maperitive: I modified some functions a bit, find my fork of the original hikingmap project here: 
* my custom scripts xx:
...
I used python to add the `<cmt>` tags. For other modifications and merging of GPX tracks, I used QMapShack (LINK)

You can find the final gpx files here(LINK)
...
For adding textures, I found this [tool](http://www.imagico.de/map/jsdotpattern.php) that allows one to create aethetically pleasing random patterns. I used this to create patterns for the textures that I wanted to add, and modified the resulting svgs a bit to my liking. The final textures can be found here (Link). Copy them to `$RENDER_OSM/Maperitive/Textures`.
...
My final Maperitive rules are available here(Link). Ensure they are available under `$RENDER_OSM/render_osm/scripts/HikingHannahHRP.mrules`. 
...
Find the final created files here(link). Note that the absolute paths inside `hm-render-maperitive-hrp-large.maperi.py` need to modified to work on another workstation.

## Export maps with Maperitive and post-process
I wrote a small script(link) to load all data and run the python script in Maperitive. Run this with
